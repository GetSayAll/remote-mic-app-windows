...
- **豆包输入法注入路径跨仓库复核（2026-09-17，用户提供 4 个参考仓库）**：实测克隆并读取以下仓库源码/文档，复核 2026-09-04 的"豆包注入判死"结论。**结论：原判定成立，获三个独立来源同向印证；无新增合规路线。** 全部引用为原文，详见 `Bugs/2026-09-17-doubao-injection-cross-repo-verification.md`。
  - `richlearntodo-debug/vibe-flow`（GPL-3.0，C#，HEAD `97fa69c`）——**独立实测确认"豆包过滤模拟输入"**：`docs/V2_0_ENHANCEMENT_RESEARCH_ZH.md` 逐字记 `豆包=无法自动化（厂商过滤模拟输入，已实测）`、`注入输入会被部分输入法过滤（豆包已实测），产品只能「检测 + 引导 + 换策略」，不能承诺绕过`；其 V2.0 已**移除豆包集成**（`docs/COMPATIBILITY_MATRIX_ZH.md`：`已按用户要求移除`）。**本仓库新采纳的线索**：其 V1.x 的豆包支持方式是**引导用户先在豆包客户端启用"全局语音快捷键"**再发送同一组合键（`docs/V1_5_USER_GUIDE_ZH.md` L110「豆包输入法 | 客户端全局语音快捷键 | 两边逐键核对」、`docs/V1_2_1_TUTORIAL_ZH.md` L84「先在豆包中启用全局快捷键」）——该路线语义上更接近系统级全局热键，是本仓库 2026-09-04 矩阵未真正覆盖的一格（当时 `enableGlobalVoiceShortcut` 是否开启需回查原始记录），已列为唯一待实测开放项。仅借鉴"先在客户端启用再用同一键"的产品引导思路，不复制实现。
    - ⚠️ **2026-09-17 命名更正**：其文档所称"全局语音快捷键"在豆包 **v0.9.0.0 界面上的真实名称是「免按模式」**（豆包设置 → 语音输入 → 语音输入模式，与「长按模式」二选一）。经三路取证：`voice.enableGlobalVoiceShortcut` 在设置 UI / ViewModel / NativeRuntime 各 DLL 与设置 exe 中三种编码全部 0 命中，仅 `ImeService.exe` 命中 1 次（无独立 UI 控件）；UI 侧对应控件为 `HandsFreeShortcutBox`，文案「按一次即可开始说话，再按任意键可结束」。**按字面找"全局"会扑空**——见 `Bugs/2026-09-17-doubao-global-shortcut-is-handsfree-mode.md`。该更正不影响其结论，但影响实验的入口设计。
    - 🔑 **同批实测补一条**：两档模式的出厂快捷键**不同**——长按模式 = 纯 右 Alt（`voiceLongPressShortcut.keyCode = 0`），免按模式 = **右 Alt + 空格**（`voiceShortcut.keyCode = 32`），`modifierFlags` 两档均为 `2049`。其文档未区分这一点。
  - `ZSTDJan/windows-remote-mic-app`，`apps/windows/rc003/src/ovb_rc003/doubao_rpc.py`（713 行）——**机理解释，不作实现参考**。其模块头逐字：`Doubao ignores the LLKHF_INJECTED flag on ordinary Win32 synthetic keyboard events... the production voice path uses an optional Frida callback hook to clear that flag inside Doubao's own low-level keyboard callback.` 做法是按模块 SHA-256 校验后硬编码 RVA `0x7426C0` attach `ImeService.exe` 的 `VoiceKeyHookProc`，在 `onEnter` 中 `flags & ~0x12` 并清 extra_info，**写回豆包进程自己的事件结构**。**这从机理上解释了本仓库 physicalize 为何无效**：本仓库是在自己进程的钩子副本上清标志（LL 钩子每钩子收到私有副本、修改不跨钩子传播、`CallNextHookEx` 无"递交修改后结构"通道）→ 结构性 no-op；其有效手法是**在豆包进程内其回调读值前改它自己的内存** → 同一块内存，豆包读到的即已清标志值。二者非同一种手法，原"physicalize 无效"判定不变；其有效手法恰被本仓库 AGENTS.md 排除（禁止第三方进程注入作为稳定语音主路径、基础路径不得依赖 Frida）。其 README L336 亦自述边界（逐字）：`如果手动快捷键都无法启动豆包，请先解决豆包快捷键或输入设备配置问题，本程序不能让本来就不工作的豆包输入法变得可用。` 即其不承诺"让不能用的豆包变得可用"；`scripts/doubao_interception_probe.py` 用 Interception 内核驱动注入，同属本仓库排除路径。
  - `QL-4/RemoteMapper`（MIT）：其语音路径为纯 SendInput 驱动 WeType（`src/RemoteMic.cs` 头注释 `holds [RAlt+Comma] for WeChat IME`），**未做豆包**——与"WeType 接受注入、豆包是注入死角"的判定相容，作侧证。（该仓库此前已在下方条目记录，本条只补豆包相关观察。）
  - `leowzz/axonkey`（**无许可证，仅作观察，不可复制代码**）——与本仓库同为 Rust + Tauri 2 + Vue，且是"遥控器 + 豆包输入法"最贴近的同类实现，其技术选择具参考价值。其默认映射把语音键映射为**右 Alt**（豆包长按热键），但下发路径是 **Interception v1.0.1 内核驱动**而非 SendInput，README 系统要求表明确写 `基础映射需要 Interception`（需管理员权限 + 重启一次），并另设"可选 Frida 增强通道"。**推论：若纯 SendInput 对豆包可用，该实现没有理由为此引入内核驱动。** 这与本仓库"纯用户态注入无法穿透豆包过滤"的判定同向，为第四个独立印证。其 `docs/RC003_AUDIO_LATENCY.md` 的 RC003 音频参数（固件 2671 / ATVV 1.0 / ADPCM 16 kHz / 120 字节帧 = 15 ms 每帧 / 实时需约 66.7 帧/秒）作为链路参考数据记录，不含可复制实现。
