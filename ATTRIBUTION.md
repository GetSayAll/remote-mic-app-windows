...
- **TSF 公开命令面**：微软 `ITfKeystrokeMgr::GetPreservedKey` / `SimulatePreservedKey` 文档说明，模拟 preserved key 的前提是目标在当前 `ITfContext` 注册了组合键并提供命令 GUID；`ITfLangBarItemMgr::EnumItems` / `ITfLangBarItemButton::OnClick` 只适用于已公开的语言栏项目。2026-09-23 本机探针在有效、聚焦的 TSF context 中激活 Chatterfly 后，Ctrl+Win 四种左右/通用修饰键查询均为未注册，枚举项也没有 Chatterfly 所属按钮。因此这两条公开 TSF 触发路线判 `failed`，不能凭 TSF profile 已激活推断存在语音命令。官方依据：`learn.microsoft.com/windows/win32/api/msctf/nf-msctf-itfkeystrokemgr-getpreservedkey`、`learn.microsoft.com/windows/win32/api/msctf/nf-msctf-itfkeystrokemgr-simulatepreservedkey`、`learn.microsoft.com/windows/win32/api/ctfutb/nf-ctfutb-itflangbaritemmgr-enumitems`、`learn.microsoft.com/windows/win32/api/ctfutb/nn-ctfutb-itflangbaritembutton`。
- **可选设备级兼容路线**：微软 Virtual HID Framework（VHF）和官方 `vhidmini2` UMDF 2 示例证明 Windows 有公开的虚拟 HID 驱动实现面；但驱动包信任、签名、安装和升级生命周期是独立的产品与发布负担，具体签名要求取决于最终驱动模型和分发方式，不能把“API 存在”写成“本项目已可分发”。若采用，只能作为用户显式安装的独立兼容组件，基础语音路径继续保持普通用户权限且不依赖它。官方依据：`learn.microsoft.com/windows-hardware/drivers/ddi/vhf/`、`github.com/microsoft/Windows-driver-samples/tree/main/hid/vhidmini2`、`learn.microsoft.com/windows-hardware/drivers/install/kernel-mode-code-signing-requirements--windows-vista-and-later-`、`learn.microsoft.com/windows-hardware/drivers/dashboard/driver-signing-offerings`。
