...
- TSF 命令面探针：`chatterfly_tsf_preserved_key_probe` 在 STA 中创建并聚焦有效 TSF context、会话级激活 Chatterfly 后，分别查询左/通用 Ctrl + 左/右 Win 四种 `TF_PRESERVEDKEY`，结果全部为 `not_registered`。同一 context 枚举到 4 个公开语言栏项目，service CLSID 均不属于 Chatterfly，描述为“语言”“更正”“键盘”“帮助”。因此 `ITfKeystrokeMgr::SimulatePreservedKey` 没有可用的 Chatterfly 命令 GUID，`ITfLangBarItemButton::OnClick` 也没有 Chatterfly 所属的公开按钮可点；这两条公开 TSF 触发路径均判 `failed`。
- 根因范围：旧版缺 Chatterfly TSF 激活已由 2b4f056 修复；当前 TSF 读回和音频路由均正常，失败收敛到 Chatterfly 对模拟快捷键的响应环节。`SendInput` 只保证事件插入输入流，不保证第三方应用处理；Chatterfly 是否过滤注入标志或只接收设备级 Raw Input 尚属推断，不能写成已证实根因。结合 TSF 命令面探针，当前已验证的公开用户态入口中没有可工作的语音触发路径。
- 修复：增加 TSF 切换前后读回日志，按公开 profile 标识分类为 `wetype`、`chatterfly`、`other` 或 `none`；保留注入及失败路径。
- 后续路线：优先请求 Chatterfly 提供公开的外部触发 API，或确认托盘菜单是否有用户可见的“开始/停止语音输入”动作；若存在，评估 Windows UI Automation。若两者均不存在，只剩显式安装的可选虚拟 HID 兼容组件能够产生设备级键盘报告；该组件不得成为基础语音路径，且驱动打包、签名、安装/卸载及失败释放仍为 `deferred`。
- 验证：TSF 读回日志、Rust 单元测试与 Tauri 仿真检查 passed；TSF preserved-key 查询与语言栏枚举探针 passed（探针完成），对应 Chatterfly 公开命令入口均 failed；本轮 RC003 触发 Chatterfly 开麦及文字上屏 failed；RC001 和其他生命周期场景 deferred。当前本地包仅用于诊断，不应宣称 Chatterfly 适配完成。
