...
> **同时运行兼容性提示：** RC003 增强采集可能无法与游戏反作弊同时正常运行，相关兼容性尚未验证。使用 PC 玩竞技类游戏的用户请注意。这里说明的是软件能否同时运行，不是说使用它会被游戏判定为作弊；目前也没有这样的实测结论。普通鼠标映射不依赖增强采集组件。详见[功能说明](docs/local-extensions.md)。
