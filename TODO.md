...
- [ ] 适配 Chatterfly：已有左 Ctrl + 左 Win 预设、CABLE 路由引导、独立目标持久化、会话级 TSF 激活和读回日志；但 2026-09-23 RC003 真机测试显示，输入法已切换、模拟快捷键已提交、音频已路由，Chatterfly 仍不开麦。公开 TSF preserved-key 与语言栏按钮探针也没有发现可调用的 Chatterfly 语音命令。下一步优先确认官方外部触发 API 或用户可见托盘动作；均不存在时再评审显式安装的可选虚拟 HID 兼容组件。完成 RC001/RC003 端到端验收后才能勾选。
