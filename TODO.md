...
- [ ] 在 Windows CI 构建较低版本 NSIS 候选，验证当前用户安装、升级后单一安装身份、设置/映射/统计逐字节保留、降级不替换当前版本和最终卸载保留用户数据；该矩阵不代表真实历史二进制或可见安装界面验收。Tauri 2.11.1 的静默页不会可靠设置内置降级检查所依赖的版本比较结果，现已在既有 preinstall hook 中增加独立 SemVer 门禁并等待 Windows CI 验证。
