...
- **冷首按原生残留（已知问题，暂不实施，方案已归档）**：武装族按键（确定/方向/主页/TV）在闲置 >4s 后的首次按压会附带一次原生按键动作（如 左→退格 冷首按 = 原生左移 + 退格；TV 附带 `` ` ``、主页附带 Home）；同键映射已由泄漏对冲解决（确定/方向冷首按与连按均单响应，2026-09-07 RC003 真机验收 passed）。用户提议的"APP 启动时主动首次触发"经调查不可行（注入事件被归因管线设计性排除；预武装 4s 过期且延长 = 劫持物理键盘）。**候选方案 A/B/C**（全局武装窗 / 同键映射持久武装 / 每键"遥控器优先"开关，可组合）与调查结论见 docs/investigations/2026-09-06-left-double-response-arm-deadlock.md"遗留问题"节——待用户选定后实施；零代价终局仍是 Helper 轨（ADR 0002）。
