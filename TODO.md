...
- [ ] 支持豆包输入法：参考 Mac App 的产品行为与配置引导，在不读取或修改豆包私有配置、内部数据库、内存或私有协议的前提下设计 Windows 支持路径；基础能力不得依赖进程注入，若必须使用提权 Helper 或虚拟 HID，须保持独立、显式启用且不影响现有语音主路径，并分别完成 RC001/RC003 真机验收。
  ...
  - **实验设计更正（2026-09-17）**：唯一未测过的合规路线"豆包的全局语音快捷键"**在 v0.9.0.0 界面上的真实名称是「免按模式」**（豆包设置 → 语音输入 → 语音输入模式，与「长按模式」二选一）。
    ...
    - `voice.enableGlobalVoiceShortcut` 在 `Settings.UI.dll` / `ViewModels.dll` / `NativeRuntime.dll` / `Settings.exe` 中**三种编码全部 0 命中**，仅 `ImeService.exe` 命中 1 次 → **无独立 UI 控件**，就是免按模式那一档的配置层内部键；本机基线 `enableGlobalVoiceShortcut = false`（即当前为「长按模式」），`hasShortcutConflict = false`。详见 `Bugs/2026-09-17-doubao-global-shortcut-is-handsfree-mode.md`。
    - **该区分对可行性判断有意义**：长按模式与按键生命周期强绑定；免按模式是"按一次开始、再按任意键结束"的**切换式**语义，意味着豆包须持续监听全局按键，**更可能走 `RegisterHotKey` 全局路径**。
  - **代码已落地（2026-09-17，提交 `58dfb9c`，分支 `feature-doubao-ime`）**：免按模式支持——新增 `DoubaoVoiceMode`（`hold` / `handsfree`）与 `InjectionShape`（`HoldWhileKeyDown` / `TogglePerKeyDown`），`ble.rs` 按下侧按形态分流（长按→`press` 并持有和弦；免按→`tap_spaced` 送一次完整点击且不记入持有态），日志新增 `shape=` 字段，前端连接页新增「语音输入模式」二选一，`voice-target.json` 新增 `doubao_mode`（`#[serde(default)]` 向后兼容）。**顺带修复一处静默缺陷**：切换输入法/开关注入时未透传 `doubaoMode` 会把已选的免按模式静默重置为长按（已用"去掉透传即回归测试变红"验证）。全量验证 passed：`sayall-windows` 143 + `src-tauri` 36 + vitest 95 用例全绿、`vue-tsc` 与 `rustfmt` 通过。
  - **🔑 缺陷修复（2026-09-17 稍晚，提交 `be6478d`）**：`default_hotkey()` 此前对豆包**无视模式**固定返回 `[RightAlt]`，`injection_shape()` 只改"怎么按"不改"按什么" → 免按模式会发送**错误的组合键**（缺空格）。已新增 `DoubaoVoiceMode::default_hotkey()` 与 `VoiceTargetConfig::default_hotkey_for_target()`，`resolved_hotkey()` 改用它，`src-tauri/src/lib.rs` 两处 snapshot 同步修正（否则界面会显示错误快捷键）。**回归测试真能捕获该缺陷**：临时把 `HandsFree` 分支改回 `[RightAlt]` → 2 个测试 FAILED，还原后逐字比对备份一致。`voice_target` 现 17 passed / 0 failed。
  - **实验进度（2026-09-17）**：协议与探针见 `Testing/investigation/doubao-global-hotkey/README.md`。
    ...
    - 阶段 1「免按模式未勾选基线」自动部分 **passed**：`RegisterHotKey` 6 候选全 FREE（`TAKEN=0`）、注入右 Alt（`--inject-rightalt`）与 `--toggle 2` 全程 `OimeVoiceWaveWindow visible=False`、录音设备 0/2 被占用。**长按语义正解形态**（`--inject-rightalt --hold-ms 1500`，按住 1.5 秒）同样 `visible=False`。
    - **deferred（真正的物理对照软件无法替代，已逐条排查）**：`osk.exe` 内部走 SendInput、`keybd_event` 同样带 injected 标记、本机**无** Interception / WinUHid 驱动（`System32` 与 `System32\drivers` 下 `.dll`/`.sys` 均不存在）→ **本机不存在非注入的按键通路**。故须 Echo 配合三步：① 长按模式下由**真人按住物理右 Alt** 做同条件对照（`--watch 30`；注入对照组会自我污染，不能代替）；② 在豆包设置 → 语音输入 → 语音输入模式 → **选中「免按模式」**（注意其快捷键是 `右 Alt + 空格`，与长按档不同）；③ 免按模式下三项测量，测完**切回「长按模式」**还原。
