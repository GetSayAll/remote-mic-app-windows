...
- [x] 适配 Chatterfly：提供左 Ctrl + 左 Win 按住说话预设和 CABLE 路由引导，并持久化区分 Chatterfly 与微信输入法的同组快捷键；按语音键时通过 Windows TSF 激活所选输入法。自动化 UI/快捷键隔离测试已通过；已加 TSF 激活后读回日志，Chatterfly 客户端与 RC001/RC003 Windows 真机端到端验收待完成。
