...
- [x] CABLE Input 端点主静音自愈：仅对识别出的虚拟 CABLE 渲染端点在打开时和每次语音会话开始前检查，必要时调用公开 Core Audio API 解除静音并读回确认；2026-09-07 Windows 真实 CABLE Input 受控复现两次均 passed（打开端点、已打开后开始会话各一次），不覆盖用户音量。边界：完整 RC001/RC003 → CABLE → 输入法语音链仍按上一项分别真机验收。
...
- **返回/音量±全型号禁用（2026-09-07 用户决策，真机验证时确认）**：此前 RC001 上三键以 VK 0xFF 厂商键可达且可直接归因（可正常映射），RC003 上输入栈不可见（格子禁用）——两型号行为不一致造成用户困惑（连 RC001 时格子放开）。决策：统一全型号禁用（RC001 也不开放），UI 格子禁用 + 持久化层/引擎层双重剥离（与左键同款机制，without_unsupported_buttons）；电源/菜单保持可配（直接归因且用户未要求禁用）。
