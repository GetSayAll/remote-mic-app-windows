...
- **冷首按原生残留（已按用户选定方案处置，2026-09-07 晚）**：武装族按键（确定/方向/主页/TV）在闲置 >4s 后的首次按压会附带一次原生按键动作（如 左→退格 冷首按 = 原生左移 + 退格；TV 附带 `` ` ``、主页附带 Home）；同键映射已由泄漏对冲解决（确定/方向冷首按与连按均单响应，2026-09-07 RC003 真机验收 passed）。用户提议的"APP 启动时主动首次触发"经调查不可行（注入事件被归因管线设计性排除；预武装 4s 过期且延长 = 劫持物理键盘）。候选方案 A/B/C 见 docs/investigations/2026-09-06-left-double-response-arm-deadlock.md"方案空间"节（含 2026-09-07 竞品 vibe-flow 调研佐证）。**已落地（用户选定组合）**：① 左键不支持自定义（策略剥离，settings 持久化层 + 映射引擎双重剥离，UI 格子禁用）；② Home/TV 落地方案 C"遥控器优先"——已配置映射且遥控器在线时常驻吞键（key_gate 常驻抑制族，跳过 60ms 有界等待零额外延迟；在线判定由 ble.rs 连接相位提交点同步，Reconnecting 视为在线、离线即恢复物理键盘透传），任意按压（含孤立冷首按）严格单响应；方向/Enter 等物理高频键不纳入（接管=劫持物理键盘）。零代价终局仍是 Helper 轨（ADR 0002）。
- RC003 按键映射真机验收 passed（2026-09-07 01:13–01:15 用户全键测试，remote-capture 逐事件比对：同键映射×4 两路径单响应、菜单直接归因、TV/主页 open_app 生效、物理键盘无劫持）；RC001 待另行真机验收。
