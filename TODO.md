...
- [ ] 在 Windows CI 构建较低版本 NSIS 候选，验证当前用户安装、升级后单一安装身份、设置/映射/统计逐字节保留、降级不替换当前版本和最终卸载保留用户数据；该矩阵不代表真实历史二进制或可见安装界面验收。当前阻塞：升级后注册表会在约 1–2 分钟内从 `0.1.0` 回落到 `0.0.1`，需先定位 NSIS/Tauri 安装器竞态。
