...
- [x] 适配 Chatterfly：提供右 Alt 按住说话预设和 CABLE 路由引导；Chatterfly 默认 Fn 属于硬件键，改用用户在 Chatterfly 内配置的右 Alt，通过公开 SendInput 注入。自动化 UI/快捷键隔离测试已通过；Chatterfly 客户端与 RC001/RC003 Windows 真机端到端验收待完成。
