...
- [ ] 在 Windows CI 构建较低版本 NSIS 候选，验证当前用户安装、升级后单一安装身份、设置/映射/统计逐字节保留、降级不替换当前版本和最终卸载保留用户数据；该矩阵不代表真实历史二进制或可见安装界面验收。此前普通 `/S` 降级路径未执行 Tauri 版本比较，现改用 `/S /P` passive 路径等待 CI 验证。
