...
当前自动化状态：第 2 项生命周期矩阵尚未通过。Windows CI 已重复观察到当前安装器先写入 `0.1.0`，随后约 1–2 分钟内回落到 predecessor `0.0.1`；等待已知旧 `uninstall.exe` 进程退出和有界 updater 重试均未消除现象。该问题定位完成前不得进入 Authenticode 发布演练或宣称升级通过。
