...
**已落地（2026-09-07 用户选定组合，真机待复验）**：左键策略性取消自定义（持久化层 + 引擎双重剥离，UI 格子禁用）；Home/TV 落地方案 C"遥控器优先"（key_gate 常驻抑制族：已映射 + 遥控器在线时常驻吞键，跳过有界等待零额外延迟；在线判定由 ble.rs 相位提交点同步，Reconnecting 视为在线、离线恢复透传）。方向/Enter 等物理高频键维持武装窗 + 泄漏对冲现状；A/B 与 Helper 轨（ADR 0002）留作后续选项。
