...
**推荐路线：在 HID 宿主内"清空报告"（源头按设备替换），即 ZSTDJan `windows-remote-mic-app` 已在正式版里跑通的形态。**
它在五个候选里是唯一同时满足"按设备 + 不装内核驱动 + 不改系统安全设置 + 不重启 + 不产生任何合成键 + 一条通道覆盖全部实体键"的方案，并且与本仓库 PR #66 已有的注入挂载点（`WUDFHost` 内 hook `NtDeviceIoControlFile`）是同一个点，只差"从只读改成写回"。
...
**必读的诚实边界**：严格意义的"零副作用"在任何路线下都不成立——每条路线都需要一个提权组件或内核组件。可达成的最优形态是：**设备级源头替换（遥控器键在进入 Windows 之前就被替换掉）+ 用户可一键停用 + 停用/失败后回到 Windows 原始行为（fail-closed）**。本文第 4 节列出推荐路线自身的残余副作用，第 6 节列出审核后必须先跑的判别实验。
...
- **覆盖面**：报告层清空，所以**同一 report ID 内的所有键**都受影响；其文档明确 tap「上报遥控器的**全部键盘 usage**（返回 `0xF1`、音量 `0x80/0x81`、方向/OK/Home/Menu/TV/Power 等）」（`apps/windows/rc003/README.md:627`）——即 Home / TV / 电源的原生动作与返回/音量± 的"看不见"用**同一条通道**一起解决。
- **按设备**：`hid_identity.py:64-88` 按 HID 接口路径的 VID/PID（`vid_2717` / `pid_32b8`）匹配，发现 0 个或多个都 fail-closed；脚本侧只用 `boundCopyHandle` / `selectedSourceKey` 接管目标句柄与来源（`frida_hid_tap_runtime.py:511-527`）。
...
- **代价与残余风险**（`apps/windows/rc003/README.md:126-160,280-283,812-831`）：产物未签名（SmartScreen 提示）；首次启用自定义映射时一次 UAC，把 HID 助手写入 Program Files 并登记固定按需计划任务，此后日常启动/自启/桥接不再弹 UAC；**要求当前登录账号本身属于管理员组**（标准账号借他人凭据不受支持）；**UAC 被拒 → 全部自定义按键映射停用、只保留 Windows 原始按键**（fail-closed，符合 R6）；Frida 注入系统宿主与游戏反作弊共存未验证（`README.md:181`）。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **A. ZSTDJan 报告层清空** | ✓ | ✓（翻译前清空） | ✓（同通道） | **未验证** | ✗ | ✗ | ✓（一次 UAC 后日常无感） | ✗ | 正式版 1.0.44 | GPL-3.0（同族） |
...
1. **R2/R3 从根上成立**：替换发生在 Windows 翻译之前，遥控器键不产生任何原生事件——没有 `` 字符、没有 Home 光标移动、没有音量变化；**TV 的 Shell 协议动作若由同一 report 引发也一并消失**（待 E1/E2 判别）。
...
5. **增量最小**：PR #66 已经把这套注入管线（提权 Helper、宿主/设备核验、租约、逐键确认、清理）建好了，只是其中一个函数是只读的；改造成"写回"是局部改动（对 `Testing/rc003-user-hid/hid_tap.js` 的报告处理加一条清空路径 + 协议上增加"接管/释放"语义）。

### 4.3 它自身引入的副作用（必须先被知情）

| 项 | 内容 | 可缓解性 |
| --- | --- | --- |
| 常驻管理员助手 | 安装时一次 UAC，写 Program Files + 登记按需计划任务；要求登录账号属管理员组 | 用户可拒绝；拒绝后自定义映射整体停用（fail-closed），基础语音不受影响 |
...
| 场景 | 普通用户看到什么 |
| --- | --- |
| 安装/首次启用 | **一次 UAC**（"把 HID 助手写入 Program Files 并登记计划任务"）；拒绝时程序仍可安装，但**全部自定义按键映射停用、只保留 Windows 原始按键**，界面显示"管理员按键组件异常 / 修复权限"（`README.md:126-127,157-159`；LEDGER:756-759） |
...
2. **杀软/企业策略拦截注入或未签名组件** → 表现为"功能突然不工作"（fail-closed，遥控器退回原始按键）。
...
| E4 | 失效与恢复 | 助手被杀 / 记录租约超时 / BLE 断连 / 睡眠唤醒 / UAC 拒绝 / 卸载 | 一律 fail-closed：立即撤钩、遥控器键回到 Windows 原始行为、**不残留清空**、不需重启 |
...
4. **fail-closed + 自动恢复**：Helper 消失必须立即回到 legacy 行为。参考实现给的量级可直接借用：Python 侧拦截租约 `HID_INTERCEPT_LEASE_SECONDS = 2.0`、续期 0.5s（`frida_compat.py:94-97`）；Gadget 侧上限 `MAX_INTERCEPT_LEASE_MS = 5000`、心跳 5s（`frida_hid_tap_runtime.py:59-61`）→ 即**"报告被清了但没人转发"的最坏窗口约 2 秒**，超过即自动撤钩。
...
| 设备归属 = ContainerID + 调用者必须是 `wudfhost.exe`；共享宿主 fail-closed | `frida_hid_tap_runtime.py:324-374,349-364,1337-1339`、`frida_compat.py:979-985` | 子代理逐行 | `passed` |
...
3. **双同型号设备**：ZSTDJan 按宿主/句柄绑定（有 fail-closed）；RemoteMapper 按硬件 ID 绑定（两台同型号都会命中）；vibe-flow 为全局单例策略。三种行为不一致，需按我们的使用场景确认。
