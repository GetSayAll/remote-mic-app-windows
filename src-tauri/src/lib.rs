...
async fn connect_remote(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let snapshot = platform
            ...
            .map_err(|error| error.to_string())?;
        // 连接后型号才确定（小米广播名不区分 RC001/RC003，需读 2A24）：
        // 同步该型号 profile 的按键映射，使按键配置跟随当前遥控器。
        if snapshot.remote_model != sayall_windows::RemoteModel::Unknown {
            let profile = sayall_windows::profile_for_model(snapshot.remote_model);
            settings.set_active_button_profile(profile)?;
            let mappings = settings.load_button_mappings_for(profile)?;
            platform.set_active_remote_model(snapshot.remote_model);
...
#[tauri::command]
fn get_button_mappings(
    profile: String,
    state: tauri::State<'_, AppState>,
) -> Result<ButtonMappings, String> {
    state.settings.load_button_mappings_for(&profile)
}
...
async fn save_button_mappings(
    ...
    let result =
        match tauri::async_runtime::spawn_blocking(move || -> Result<ButtonMappings, String> {
            let saved = settings.save_button_mappings_for(&profile, mappings)?;
...
async fn reset_button_mappings(
    ...
    let result =
        match tauri::async_runtime::spawn_blocking(move || -> Result<ButtonMappings, String> {
            let saved = settings.save_button_mappings_for(&profile, ButtonMappings::default())?;
            platform.set_button_mappings(saved.clone());
            Ok(saved)
        })
        .await
...
async fn export_button_mapping_configuration(
    ...
    let result = match tauri::async_runtime::spawn_blocking(move || -> Result<bool, String> {
        ...
        settings.export_button_mappings_for(&profile, &path, mappings)?;
        Ok(true)
    })
    .await
    {
...
async fn import_button_mapping_configuration(
    ...
    let result = match tauri::async_runtime::spawn_blocking(
        move || -> Result<Option<ButtonMappings>, String> {
            ...
            let imported = settings.import_button_mappings_for(&profile, &path)?;
        ...
        .setup(|app| {
            ...
            let button_mappings = match active_profile.as_deref() {
                Some(profile) => match settings.load_button_mappings_for(profile) {
                    Ok(mappings) => {
                        sayall_windows::gatt_note(format!(
                            "shortcut_settings feature=button_mapping action=load phase=completed terminal_result=passed profile={profile} {}",
                            button_mapping_log_summary(&mappings)
                        ));
                        platform.set_active_remote_model(
                            sayall_windows::remote_model_from_profile(profile),
                        );
                        mappings
                    }
                    Err(error) => {
